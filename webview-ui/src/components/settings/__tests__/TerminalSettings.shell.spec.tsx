...
describe("TerminalSettings inline shell selector", () => {
	...
	const setup = (options?: {
		...
		return { ...utils, setCachedStateField }
	}
	...
	it("sends setTerminalShellSelection when Auto is selected", () => {
		setup({ terminalShellIntegrationDisabled: true })
	...
	it("sends setTerminalShellSelection with profile name when a profile is selected", () => {
		...
		const payload: TerminalShellOptionsPayload = {
			options: [
				...
				{ id: "profile:PowerShell", label: "PowerShell", family: "powershell", source: "vscode-profile", available: true },
			],
			effectiveShell: {
				label: "pwsh.exe",
				family: "powershell",
		...
		act(() => {
			window.dispatchEvent(new MessageEvent("message", { data: { type: "terminalShellOptions", terminalShellOptions: payload } }))
		})
