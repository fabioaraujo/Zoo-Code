...
export class ShellResolver {
	 ...
	 */
	resolve(
		settings: ShellResolverSettings,
		cliOverride?: string,
	): ShellResolutionResult {
		...
		if (settings.terminalShellSelection?.kind === "path") {
			const result = this.tryResolveExplicitPath(
				settings.terminalShellSelection.path,
				"userOverride",
			)
			if (result.ok) {
				return result
		...
		if (settings.terminalShellSelection?.kind === "profile") {
			const result = this.tryResolveProfile(
				settings.terminalShellSelection.profileName,
				"userOverride",
			)
			if (result.ok) {
				return result
	 ...
	 */
	private tryResolveExplicitPath(
		shellPath: string,
		...
		// Pass the original shellPath, not the platform-normalized version,
		// because isShellPathAllowed tries both path.normalize and
		// path.posix.normalize internally for cross-platform compatibility.
		if (!isShellPathAllowed(shellPath)) {
			return this.fail(
		...
		// the original path for cross-platform compatibility.
		if (!this.fs.existsSync(normalizedPath) && !this.fs.existsSync(shellPath)) {
			return this.fail(
		...
		// Classify the shell family. Use the original path for classification
		...
		return {
			...
			shell: {
				...
				family,
				displayName: this.deriveDisplayName(family, shellPath),
				source,
	...
	private tryResolveDefaultProfile(source: ShellResolutionSource): ShellResolutionResult {
		...
		if (!shell) {
			return this.fail(
				"SHELL_PROFILE_NOT_FOUND",
				"No VS Code default terminal profile configured.",
				false,
			)
		}
	...
	private fail(
		code: ShellResolutionError["code"],
		message: string,
		rejectable: boolean,
	): ShellResolutionResult {
