...
describe("terminal-shell-messages — webview message handlers", () => {
	...
	beforeEach(() => {
		...
		resolveMock.mockReturnValue({
			ok: true,
			shell: { executable: "/bin/bash", family: "posix", displayName: "bash", source: "osDefault", trustEvidence: "allowlist" },
		})
		...
		mockProvider = {
			...
			handleRequestTerminalShellOptions: vi.fn().mockImplementation(async function (this: any) {
				...
				const options: TerminalShellOption[] = [
					{ id: "auto", label: "Auto (follows trusted terminal profile)", family: "powershell", source: "auto", available: true },
				]
			...
			handleSetTerminalShellSelection: vi.fn().mockImplementation(async function (this: any, selection: TerminalShellSelection) {
				const svc = this.getCommandEnvironmentService()
				if (!svc) {
					await this.postMessageToWebview({
						type: "terminalShellOptions",
				...
				if (!result.ok && result.rejectable) {
					await this.postMessageToWebview({
						type: "terminalShellOptions",
						terminalShellOptions: { options: [], error: `SHELL/handleSetTerminalShellSelection/003: ${result.error.message}` },
					})
					return
				}
	...
	describe("requestTerminalShellOptions", () => {
		...
		it("includes trusted profile options grouped by shell family", async () => {
			getAvailableProfilesMock.mockReturnValue([
				{
					name: "PowerShell",
					shell: { executable: "pwsh.exe", family: "powershell", displayName: "PowerShell 7", source: "vscodeDefaultProfile", trustEvidence: "trustedProfile" },
				},
				{
					name: "Git Bash",
					shell: { executable: "/usr/bin/bash", family: "posix", displayName: "Git Bash", source: "vscodeDefaultProfile", trustEvidence: "trustedProfile" },
				},
			])
	...
	describe("setTerminalShellSelection", () => {
		...
		it("returns typed error and keeps previous setting on validation failure", async () => {
			...
			resolveMock.mockReturnValue({
				ok: false,
				error: { code: "SHELL_PATH_NOT_ALLOWED", message: "The selected shell path is not in the trusted allowlist: evil.exe" },
				rejectable: true,
			})
