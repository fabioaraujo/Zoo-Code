	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	...
	*/
	...
	public async handleRequestTerminalShellOptions(): Promise<void> {
		try {
			...
			const effectiveShell = {
				...
				family: env.primaryPlan.family as
					| "powershell"
					| "cmd"
					| "posix"
					| "fish"
			...
			const message =
				error instanceof Error ? error.message : String(error)
			console.error(
				`[ClineProvider] SHELL/handleRequestTerminalShellOptions/002: ${message}`,
			)
			await this.postMessageToWebview({
				type: "terminalShellOptions",
				terminalShellOptions: {
	...
	/**
		...
		* See ARCH-TERMINAL-001 section 1.9 (Frontend to extension-host settings flow).
		*/
	public async handleSetTerminalShellSelection(
		...
		try {
			...
			const message =
				error instanceof Error ? error.message : String(error)
			console.error(
				`[ClineProvider] SHELL/handleSetTerminalShellSelection/002: ${message}`,
			)
			await this.postMessageToWebview({
				type: "terminalShellOptions",
				terminalShellOptions: {
	...
	/**
		...
		*/
	private buildTerminalShellOptions(
